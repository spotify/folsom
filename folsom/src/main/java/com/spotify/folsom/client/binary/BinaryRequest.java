...
public abstract class BinaryRequest<V> extends Request<V> {
  ...
  // This is racy but it doesn't really matter,
  // it's only used to detect misbehaviour in the protocol
  private static int opaqueCounter = 0;
  ...
  protected BinaryRequest(final byte[] key) {
    super(key);
    opaque = (opaqueCounter++ << 8) & 0xFFFFFF00;
  }
